...
object fp3 {
  ...
  def member (a : Int, xs : List[Int]) : Boolean = {
    ...
    xs match{
    	...
    	case x::xs => if (x==a)
    						true
    						else member(a, xs)// also, (x==a) || member(a,xs)
    }
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    // TODO: Provide definition here.
    def aux [X] (y:X, ys:List[X], count:Int): List[(Int,X)] ={
    	ys match{
    		case Nil => if (y==Nil)
    						Nil 
    						else 
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    xs match{
    	case Nil => (Nil,Nil)
    	case x::xs=>
    				val tmp=splitAt(n-1, xs)
    				if (n>0) (x::tmp._1, tmp._2)
    					else 
    						(tmp._1, x::tmp._2)
