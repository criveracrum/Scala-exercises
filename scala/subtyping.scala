...
object subtyping {
  ...
  class CountObserve extends Counter{
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    // TODO: Provide definition here.
    val count = new CountObserve
    val count2 = new CountObserve
    val count3 = new CountObserve
    f (List(count, count2, count3))
    List(count.get(), count2.get(), count3.get())
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    ...
    val count2 = new CountObserve
    val count3 = new CountObserve
    val array = List(count, count2, count3)
    f(array.toArray)
    List(count.get(), count2.get(), count3.get()).toArray
